      ...
      enumerateObjectsUsingBlock:^(IDEFileNavigableItem *fileNavigableItem,
        ...
        if ([document
          ...
          [self formatRanges:@[
            [NSValue
                valueWithRange:
                    NSMakeRange(0, [[sourceCodeDocument textStorage] length])]
...
- (void)formatDocument:(IDESourceCodeDocument *)document {
  ...
  NSUInteger diff =
      MAX(length, textStorageLength) - MIN(length, textStorageLength);
      ...
      fragmentsOfContinuousLineRanges:continuousLineRanges
                     usingTextStorage:textStorage
                         withDocument:document
                                block:^(NSArray *fragments,
                                        NSArray *changedStates,
                                        NSArray *errors) {
                                  if (errors.count == 0) {
                                    NSArray *selectionRanges = [self
                                        selectionRangesAfterReplacingFragments:
                                            fragments changedStates:
                                                          changedStates
                                                              usingTextStorage:
                                                                  textStorage
                                                                  withDocument:
...
selectionRangesAfterReplacingFragments:(NSArray *)fragments
                         changedStates:(NSArray *)changedStates
  ...
  [fragments enumerateObjectsUsingBlock:^(TRVSCodeFragment *fragment,
                                          NSUInteger idx, BOOL *stop) {
    if ([changedStates[idx] boolValue]) {
...
- (void)fragmentsOfContinuousLineRanges:(NSArray *)continuousLineRanges
  ...
  NSMutableArray *changedStates = [NSMutableArray array];
  ...
  [continuousLineRanges enumerateObjectsUsingBlock:^(NSValue *rangeValue,
    ...
    [fragment formatWithStyle:self.style
        usingClangFormatAtLaunchPath:executablePath
                           lineRange:lineRange
                               block:^(NSString *formattedString, BOOL changed,
                                       NSError *error) {
                                 __strong typeof(weakFragment) strongFragment =
                                     weakFragment;
                                 if (error) {
                                   ...
                                   *stop = YES;
                                   ...
                                   [changedStates addObject:@(changed)];
                                 }
                               }];
  }];

  block(fragments, changedStates, errors);
}
...
- (NSArray *)continuousLineRangesOfRanges:(NSArray *)ranges {
  ...
  [ranges enumerateObjectsUsingBlock:^(NSValue *rangeValue, NSUInteger idx,
                                       BOOL *stop) {
